...
Claude Code に `setup.md` を渡すと、10ステップで基盤構築が完了します。
...
| `templates/CLAUDE.md` | Claude Code 用のroot入口ブリッジ |
| `templates/.claude/AGENTS.md` | Codex が `.claude/` 配下で作業する時の入口ブリッジ |
| `templates/.claude/CLAUDE.md` | Claude Code 用のプロジェクト固有入口テンプレート |
| `templates/.claude/constitution.md` | 不変原則10条（全プロジェクト共通） |
| `templates/.claude/CURRENT_CONTEXT.template.md` | 事業の現在地テンプレート（新AIセッションが最初に読むファイル） |
| `templates/.claude/02_specs/README.md` | Spec-Driven ワークフロー + テンプレート3種 |
| `templates/.claude/02_specs/DEPRECATION_GUIDE.md` | Spec状態マーキングガイド（廃止・実装済み・統合・アーカイブ） |
...
| `templates/agent/prompts/` | 開発プロンプトライブラリ |
...
| `CLAUDE.md` | root入口ブリッジ。通常は `templates/CLAUDE.md` をコピーし、プロジェクト名だけ調整 |
| `docs/INDEX.md` | ドキュメント一覧（プロジェクトの docs/ に依存） |
| `.claude/CLAUDE.md` | サービス概要、料金、ルール。`templates/.claude/CLAUDE.md` を元に生成 |
| `.claude/AGENTS.md` | `.claude/` 配下で Codex が作業する時の入口。通常は `templates/.claude/AGENTS.md` をコピー |
...
```
templates/
├── AGENTS.md                            ← 全AI共通root入口テンプレート
├── CLAUDE.md                            ← Claude Code root入口ブリッジ
├── .claude/AGENTS.md                   ← Codex向け .claude 入口ブリッジ
├── .claude/constitution.md             ← 10の不変原則
├── .claude/CURRENT_CONTEXT.template.md ← 事業の現在地（セッション再現性）
├── .claude/02_specs/README.md          ← specify → plan → tasks フロー
├── .claude/02_specs/DEPRECATION_GUIDE.md ← 廃止specのマーキングルール
├── tasks/lessons.md                    ← ミス記録 → 再発防止
├── .claude/hooks/            ← AI の書き込みを自動チェック
├── .husky/                   ← git commit 時の自動チェック
└── commitlint.config.js      ← コミットメッセージ規約

templates/agent/prompts/      ← 開発プロンプト（投げまくって完成させる）

setup.md                      ← AI が読んで実行する10ステップ手順
```
...
詳細は [`templates/agent/prompts/README.md`](templates/agent/prompts/README.md) を参照。
