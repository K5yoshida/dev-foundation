 ...
 *   npx tsx .agent/orchestrate.ts auto                     # 完全自動（next をループ）
 */
...
async function main(): Promise<void> {
  ...
  // "auto" — 完全自動（next をループで回し続ける）
  if (cliArgs.task === 'auto') {
    const maxIter = config.autoLoop.maxIterations
    log('info', 'orchestrate', `=== 完全自動モード開始（最大 ${maxIter} 回） ===`)

    for (let i = 0; i < maxIter; i++) {
      // 毎回予算チェック
      const loopBudgetIssue = checkBudget(config.paths.budgetFile, config.budget)
      if (loopBudgetIssue) {
        log('warn', 'orchestrate', `予算到達で自動停止: ${loopBudgetIssue}`)
        break
      }
      ...
      log(
        'info',
        'orchestrate',
        `[${i + 1}/${maxIter}] 自動判定: ${routing.prompt} — ${routing.reason}`,
      )
      ...
      if (!result.success) {
        log('error', 'orchestrate', `自動停止: ${routing.prompt} が失敗 — ${result.output}`)
        break
      }

      log('info', 'orchestrate', `[${i + 1}/${maxIter}] ${routing.prompt} 完了`)
    }

    log('info', 'orchestrate', '=== 完全自動モード終了 ===')
    return
  }
